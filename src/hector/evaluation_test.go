...
func TestAUC(t *testing.T){
	...
	for i := 0; i < 1000; i++ {
		predictions = append(predictions, &(LabelPrediction{Label: float64(rand.Int() % 2), Prediction: rand.Float64()}))
	}
	auc := AUC(predictions)
...
func TestRMSE(t *testing.T){
	predictions := []*LabelPrediction{}
	for i := 0; i < 1000; i++ {
		predictions = append(predictions, &(LabelPrediction{Label: 1.0, Prediction: 1.0}))
...
func TestErrorRate(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		p := rand.Float64()
		predictions = append(predictions, &(LabelPrediction{Label: p, Prediction: p}))
	}

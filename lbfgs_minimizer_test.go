...
func (f *mseDiffFunction) getValue(x *Vector) float64 {
    var val float64 = 0
    for n, val := range x.data {
		diff := val - f.center.GetValue()
...
func (f *mseDiffFunction) getGradient(x *Vector) *Vector {
    for n, val := range x.data {
        f.grad.SetValue(f.weights.GetValue(n) * (val - f.center.GetValue(n)))
    }
    return f.grad
}	
